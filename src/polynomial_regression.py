...
class PolynomialRegression():
    ...
    def fit(self, features, targets):
        ...
        """
        X = np.empty([len(features), self.degree + 1])
        ...
        w = np.linalg.inv(w)
        w = np.matmul(w, X_T)
        targs = np.transpose(targets)
        weights = np.matmul(w, targs)
    ...
    def predict(self, features):
        ...
        for i in range(0, len(features)):
            predictions[i] = poly(features[i], self.weights, self.degree)
    ...
    def visualize(self, features, targets):
        ...
        """
        plt.plot(features,self.predict(features),'rs')
